...
solver_a_bit_smarter <- function(history){
  ...
  nb_white <- history[iter,"nb_white"]
  nb_red <- history[iter,"nb_red"]
  
  if(iter>1 & (nb_white + nb_red)>0 ){ browser()}
  ...
  combn_n_white <- combn_n_white %>% t %>% 
    ...
    group_nest(id) %>%
    mutate(
      position = map(data,unlist),
      values = map(position,~last_try[.x])
      # patterns = map2(.x = position, .y = values, .f=~browser() )
      ) |> 
    select(-data)
  ...
  all_comb |> 
    mutate(
      # weight = case_when(
      #   toto %in% c(pawn1,pawn2,pawn3,pawn4) 
      # )
    )
